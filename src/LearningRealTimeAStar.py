...
def A_star_search():
    global f_cost, g_cost, h_cost, open, closed, action, policy, grid
    ...
    while(not found and not resign):
        ...
        if len(open) == 0:
            ...
            if x == goal[0] and y == goal[1]:
                open.append([f_cost[x][y], g_cost[x][y], h_cost[x][y], x, y])
                found = True
            else:
...
def learning_real_aa():
    global f_cost, g_cost, h_cost, open, closed, action, policy, movements, grid
            ...
            if(gui.isplay):
                ...
                for i in range(len(closed)):
                    ...
                    for j in range(len(open)):
                        ...
                        #print("Shortest Distance", x,y, x1,y1, dist, h_cost[x1][y1])

                        min_value = min(dist + h_cost[x1][y1], min_value)
                    #print("Modified h value--",x,y,h_cost[x][y], min_value)
...
AStar_thread = threading.Thread(target = learning_real_aa)
